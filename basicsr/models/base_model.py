...
class BaseModel():
    ...
    def gather_tensors(self, tensor_to_gather):
        if tensor_to_gather is None:
            return None
        group = torch.distributed.group.WORLD
        group_size = torch.distributed.get_world_size(group)
        gather_t_tensors = [torch.zeros_like(tensor_to_gather) for _ 
                            in range(group_size)]
        torch.distributed.all_gather(gather_t_tensors, tensor_to_gather)
        return torch.cat(gather_t_tensors, dim=0)
