...
def clipped_softmax(tensor, dim=-1):
    """
    Computes a softmax over the specified dimension while ignoring zero elements in the tensor.
    """
...
##########################################################################
# Gated Feed-Forward Network similar to Restormer paper
...
class StateAlignBlock(nn.Module):
    ...
    def forward(self, x, k_cached=None, v_cached=None):
        ...
        H = int(q.shape[2] / self.window_size)
        W = int(q.shape[3] / self.window_size)
        
        # Rearrange inputs into windows and split into multiple heads in one step
        q = rearrange(q, 'b (h_head d) (p1 h) (p2 w) -> b 1 h_head (h w) (p1 p2 d)', 
                      h_head=self.num_heads, p1=self.window_size, p2=self.window_size, d=head_dim)
        k = rearrange(k, 'b (h_head d) (p1 h) (p2 w) -> b 1 h_head (h w) (p1 p2 d)', 
                      h_head=self.num_heads, p1=self.window_size, p2=self.window_size, d=head_dim)
        ...
        # Concatenate cached k and v if they exist, Updating the History states with the current frame
        if k_cached is not None and v_cached is not None:
        ...
        # Combine the two attention mechanisms(Top-k attention score and attending to neighboring patches) and apply softmax
        attn = (attn1 + attn2)/2
...
class TurtleAttnBlock(nn.Module):
    def __init__(self, dim, ffn_expansion_factor, bias, LayerNorm_type, num_heads=1, Scale_patchsize=1, attention_type='channel', FFW_type="GFFW", num_frames_tocache=1, plot_attn=False):
        ...
        """
        This class gives you the freedom to design custom transfomer blocks with the abilty to 
...
class Turtle(nn.Module):
    ...
    def forward(self, inp_img_, k_cached=None, v_cached=None):
        ...
        #initializaing KV cache to save the history of the frames
        k_to_cache = []
        v_to_cache = []
